...
with st.form(key='form_py_code2', border=False):
    # Pregunta de usuario
    question_py_code2 = st.text_input(placeholder='Escribe una consulta en lenguaje natural para convertirla en código', key='input_py_code2', label='', label_visibility='collapsed')
...
if btn_py_code2 and question_py_code2:
    ...
    with st.spinner('El LLM está procesando tu consulta...'):
        ...
        response_py_code2 = gemini.models.generate_content(model=GEMINI_MODEL, contents=prompt_py_code2)
        response_py_code2 = response_py_code2.text

    # 6. RESULTADO DEL LLM
    st.success('Código generado')
    st.write(response_py_code2)
